...
def parse_option():
    ...
    parser.add_argument("--save_interval", type=int, default=1, help="save frequency")
    parser.add_argument(
...
def get_model_resgcn(graph, graph2, opt):
    model_args = {
        "A": torch.tensor(graph.A, dtype=torch.float32, requires_grad=False),
        "A2": torch.tensor(graph2.A, dtype=torch.float32, requires_grad=False),
        "num_class": opt.embedding_layer_size,
        "num_input": 1 if not opt.use_multi_branch else 4,
